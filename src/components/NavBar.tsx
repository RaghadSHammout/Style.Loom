...
<div className={`font-robotmono w-full flex justify-between items-center font-normal text-lg leading-[1.5] opacity-100 tracking-normal text-center p-[30px] relative  max-2xl:p-6 max-lg:pt-10 max-lg:pb-5 max-lg:px-0 max-2xl:text-sm ${
...
<div className={`flex gap-3.5 justify-center items-center ${menuOpen ?  "max-lg:flex " : "max-lg:hidden"}`}>
    <button
     className={`bg-dark-10 text-white shadow-xl  hover:-translate-y-0.5 p-[18px] rounded-[12px] opacity-100 flex justify-center items-center max-2xl:p-[14px] max-2xl:rounded-[8px] max-2xl:h-[48px]  max-2xl:w-[48px]  ${menuOpen ?  "max-lg:h-[56px]  max-lg:w-[56px]  " : ""}`}
     onClick={OpenPop}>
     {market}
    </button>
    <button className={`${menuOpen ? "  flex justify-center items-center  transition-all duration-300 ease-in-out w-full max-lg:flex max-lg:flex-col max-lg:absolute  max-lg:top-[311px]  max-lg:left-0 right-0 max-lg:bg-dark-10/85 backdrop-blur-lg  max-lg:shadow-2xl  max-lg:py-5 max-lg:px-5  max-lg:z-50   " : "max-lg:hidden"} `}>
    <NavLink
              to={"/contact"}
...
<div className={`w-[70px] h-[70px] relative flex justify-center items-center ml-[30px]  max-2xl:w-[46px]  max-2xl:h-[46px]  max-2xl:ml-5 max-xl:hidden  ${scrolled ? "hidden" : ""}`}>
