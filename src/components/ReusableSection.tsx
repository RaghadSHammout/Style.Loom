  ...
  return (
    <section className=" border-2 border-dashed border-dark-15 ">
      ...
      >
        ...
        >
          <h2
            className={`font-medium text-5xl leading-[100%] tracking-normal uppercase max-2xl:text-[38px] max-lg:text-[28px] `}
          >
            {heading}
          </h2>
          <p
            className={`text-gray-40 font-normal text-lg   leading-[150%] tracking-normal max-2xl:text-base max-lg:text-sm `}
          >
            {text}
          </p>
        </div>
        {image && (
          <div
            className={`absolute right-0 top-0 overflow-hidden max-lg:hidden `}
          >
            <img
              src={image}
              alt={alt}
              className={` overflow-hidden ${imgwidth} `}
            />
          </div>
        )}
